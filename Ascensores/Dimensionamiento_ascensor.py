AA = input("Indique el grupo de asensores a trabajar: ")
...
pn = int (input ("Ingrese la capacidad nominal de personas, de los ascensores: ")) #capacidad nominal de los ascensores.
B_i = int (input ("N° de pasajeros por piso: ")) #Numero universal para todos los proyectos.
...
B = ((B_i*n) + (Zot*15)) #n_a va a variar segun sea el piso maximo a donde llegue las ascensores.
...
if Q < V_n: #Condiciones para poder calcular el tiempo de viaje.
    ...
    TVC = ((2*H_a)/(Q))+(((V_n/1)+T_1)*(n_p+1))-((H_s/(n_p*(V_n)))+((T_2)*(P_v)))
